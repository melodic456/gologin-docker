...
def request_for_django_api(path="", method="GET", data=None):
    ...
    url = "http://" + sys.argv[1] + "/api/v1/proxymodel/" + path
...
def brower_get_main(profile):
    try:
        ...
        if platform == "linux" or platform == "linux2":
            chrome_driver_path = "./root/goLogin-selenium/chromedriver"
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as tpe:
        ...
        results = [tpe.submit(brower_get_main, profile_id_) for profile_id_ in profile_ids]
